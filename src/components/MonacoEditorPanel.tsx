...
export function MonacoEditorPanel({ code, isReadOnly, sabotageEffect, onChange }: MonacoEditorPanelProps) {
  const containerVariants = {
    scramble: {
      x: [0, -6, 6, -4, 4, -2, 2, 0],
      transition: { duration: 0.5, repeat: 4, repeatType: "loop" as const },
    },
    reverse: { scaleX: -1, transition: { duration: 0.3 } },
    normal: { x: 0, scaleX: 1 },
  };

  const effect = sabotageEffect === "scramble" ? "scramble"
    : sabotageEffect === "reverse" ? "reverse"
    : "normal";

  return (
    <motion.div
      className="w-full h-full rounded-lg overflow-hidden border border-[rgba(57,255,20,0.2)]"
    ...
    >
      {/* Top bar */}
      <div className="flex items-center gap-2 px-4 py-2 bg-[rgba(57,255,20,0.05)] border-b border-[rgba(57,255,20,0.15)]">
      ...
      <Editor
        ...
        value={code}
        onChange={isReadOnly ? undefined : (v) => onChange?.(v ?? "")}
        options={{
          readOnly: isReadOnly,
